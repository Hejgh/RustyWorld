...
pub enum MobState {
    ...
    Explode { fuse: Timer }, // Bomber only
}
...
pub fn spawn_mobs(
    ...
    // Only spawn at night or during special events
    ...
    // Spawn boss on full moon with 28% chance
    ...
    // Regular mob spawning (once per 200 ticks, within 32-64 blocks)
...
fn spawn_bomber(commands: &mut Commands, pos: Vec3) {
    commands.spawn((
        Mob {
            ...
            max_health: 40.0,
            speed: 1.5,
            attack_damage: 50.0, // Explosion damage
            attack_range: 2.5,
...
fn spawn_deathwish(commands: &mut Commands, player_pos: Vec3, world_chunks: &WorldChunks) {
    let spawn_pos = random_surface_position(player_pos, world_chunks);
    commands.spawn((
        Mob {
            health: 500.0,
            max_health: 500.0,
...
fn random_surface_position(around: Vec3, world_chunks: &WorldChunks) -> Vec3 {
    let mut rng = rand::thread_rng();
    let x = around.x + rng.gen_range(-32.0..32.0);
    let z = around.z + rng.gen_range(-32.0..32.0);
    
    // Simplified: find top block at this X,Z
    let y = 70.0; // In real impl, raycast to find surface
...
// ============ AI STATE MACHINE ============
pub fn update_mobs(
    mut mobs: Query<(&mut Mob, &mut Transform, Option<&Bomber>, Option<&DeathWish>)>,
    player_query: Query<&Transform, With<Player>>,
    time: Res<Time>,
    mut commands: Commands,
) {
    ...
    for (mut mob, mut transform, is_bomber, is_boss) in mobs.iter_mut() {
        ...
        match &mut mob.state {
            MobState::Idle { wander_target, timer } => {
                ...
                if distance < 32.0 {
                    mob.state = MobState::Chase { target: player_query.iter().next().unwrap(), last_seen: player_transform.translation };
                }
            }
            
            MobState::Chase { target, last_seen } => {
            ...
            MobState::Explode { fuse } => {
                ...
                if fuse.just_finished() {
                    // Explosion destroys blocks and hurts entities
                    println!("💥 BOMBER EXPLODES at {:?}", transform.translation);
                    commands.entity(mobs.iter_mut().next().unwrap().0).despawn();
                }
            }
        }
